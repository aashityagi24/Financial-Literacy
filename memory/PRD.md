...
### Learning-First Dashboard Hero — Grade K-3 (Feb 2026) ✅
- New `GET /api/content/next-lesson` backend endpoint (`routes/content.py`, children only, 403 otherwise): walks the curriculum tree (topics → subtopics → content items, all grade/curriculum-scoped and in `order`) using the same helpers as `GET /content/topics` (`find_with_grade_order`, `apply_grade_overrides`, `get_classroom_done_content_ids`, `is_user_in_test_mode`) and returns the first incomplete, unlocked content item — the child's real "next lesson". For a brand-new user with zero progress this naturally resolves to the very first item of the first subtopic of the first topic. Also returns `completed_today` (distinct content items completed today, UTC date) and a hardcoded `daily_goal: 3`. Returns `{content_id: null, all_done: true, ...}` when everything is finished.
- `Dashboard.jsx`: for `grade <= 3` only, inserted a new top-of-page section (above "Quick Navigation") with (1) a big gradient hero card (`#FFD23F`→`#EE6C4D`, existing CoinQuest palette) showing "Your First Lesson"/"Continue Learning", the real lesson title/topic/subtopic/XP reward, and a "Start Learning"/"Continue" CTA that deep-links to `/learn/topic/{subtopic_id}?highlight={content_id}` (reuses TopicPage's existing highlight-and-scroll behavior for a clear next action); and (2) a "Today's Goal" card with an SVG circular progress ring (teal `#06D6A0`) showing real `X/3` lessons completed today. The pre-existing "Learn" quick-nav tile is removed for grade ≤ 3 (now redundant with the hero's CTA) — Grade 4-5 dashboard is completely unchanged (old layout, "Learn" tile still present).
- Tested by testing_agent (iteration_109): 5/5 new backend pytest cases pass (`test_next_lesson.py` — shape, 403 non-child, 401 unauth, `completed_today` increments after completion, `is_new_user` flips false) + full Playwright pass (hero renders real data, CTA navigation + highlight works end-to-end, ring shows correct count, Grade 3 nav has no Learn tile, Grade 4 regression confirmed unchanged, no mobile overflow). Zero bugs found. Temporary Grade 4 QA account `classmate_g4_qa`/`testpass123` created for the regression check (see test_credentials.md).
