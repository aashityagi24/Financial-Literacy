...
- **Money-skills icon fix**: "Earning" card now shows ₹ (rupee sign), "Saving" shows 🐷 (pig face), replacing the generic dollar-themed emoji.
- **Scroll-restoration bug fix**: clicking "Guided Workshop" / "DIY Platform" (or any cross-page nav) previously landed on the destination page mid-scroll instead of at its hero, because React Router preserves window scroll position across client-side route changes. Fixed with a global `ScrollToTop` component (`App.js`, keyed on `location.pathname`) that resets scroll to top on every route change while leaving same-page anchor links (`#programs`, `#features`, `#batches`) and browser back-button scroll restoration untouched. Verified by testing_agent: 9/9 scrollY checks passed (iteration_101.json).
- **Hub hero pass 3**: Platform "how it works" steps 2 & 3 renamed/refocused ("Watch them learn" → multi-format learning; "Implement at home" → parent habit-building/money tracking). Trust element now uses 3 AI-generated photorealistic Indian-parent portraits instead of plain color dots. Hero simplified to a single "Book a Call" CTA (removed the two prior buttons) that opens an in-page modal form (name, Indian-mobile-validated phone, email, program select, parent/school, child grade) via new reusable `components/BookCallDialog.jsx`.
- Backend: new public `POST /api/subscriptions/call-request` (Indian phone regex validation) + `db.call_requests` collection; admin `GET/PUT/DELETE .../admin/call-requests[...]` incl. bulk-delete (registered before the generic `/admin/{subscription_id}` catch-all to avoid the same route-shadowing bug hit earlier with trial-enquiries).
- Admin: new "Call Requests" tab in `AdminPage.jsx` mirroring the Trial Requests / Enquiries pattern (status pipeline, CSV export, single/bulk delete).
