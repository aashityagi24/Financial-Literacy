...
HERO_IMAGE_KEY = "landing_hero_image"


@router.get("/settings/landing-hero-image")
async def get_landing_hero_image_admin(request: Request):
    """Return the current landing hero image URL (public — the landing page
    fetches this before login to render the first-fold image)."""
    db = get_db()
    doc = await db.site_settings.find_one({"key": HERO_IMAGE_KEY}, {"_id": 0})
    return {"image_url": doc.get("value") if doc else None}


@router.put("/settings/landing-hero-image")
async def update_landing_hero_image(request: Request):
    """Admin: set the landing hero image URL. Body: `{image_url: str}`.
    The frontend uploads the (already-cropped) file via `/upload/image` and
    passes the returned URL here.
    """
    from services.auth import require_admin
    db = get_db()
    await require_admin(request)
    ...
    return {"message": "Landing hero image updated", "image_url": image_url}


@router.delete("/settings/landing-hero-image")
async def delete_landing_hero_image(request: Request):
    """Admin: reset the landing hero image to the built-in default."""
    from services.auth import require_admin
    db = get_db()
    await require_admin(request)
    await db.site_settings.delete_one({"key": HERO_IMAGE_KEY})
    return {"message": "Landing hero image reset to default"}
